...
每 4 小时随机展示一条自己的微信读书划线；点击 Widget，可直接跳回微信读书中的原文位置。
...
它会从你自己的微信读书划线中随机抽取一条，在桌面或锁屏上展示。到了下一个 4 小时时段，会重新随机一次。
...
| ⏱️ 每 4 小时更新 | 00–04、04–08、08–12、12–16、16–20、20–24 六个时段 |
...
同一 4 小时时段内会使用缓存，因此系统多次刷新 Widget 时不会不断换句子。进入下一个时段后重新随机，属于**独立、有放回抽样**。
...
每进入一个新时段，脚本会重新进行一次随机抽取。

需要注意：iOS 对 Widget 后台刷新拥有最终调度权。脚本会通过 <code>refreshAfterDate</code> 请求在下一个时段开始后刷新，但**实际刷新时间可能被 iOS 延后**。

因此这里的“每 4 小时”指的是**每个 4 小时时段对应一条固定摘录**，而不是保证系统在整点精确执行刷新。
